...
// require local state.
//
// It also installs /etc/profile.d/cw-claude-token-shadow.sh, which unsets
// CLAUDE_CODE_OAUTH_TOKEN whenever ~/.claude/.credentials.json is present.
// Without this, a `claude auth login` flow run inside the VM mints a
// full-scope session token to the credentials file, but the long-lived
// (inference-only) env var still wins on the next claude invocation —
// which silently breaks Remote Control and any other feature that
// requires full scope. See cli issue #7.
//
...
`, workdir, containerHome, claudeTokenShadowScript())
...
// claudeTokenShadowScript returns the contents of
// /etc/profile.d/cw-claude-token-shadow.sh. It runs on every interactive
// shell startup. When the user has minted full-scope credentials by running
// `claude auth login` inside the VM (writing ~/.claude/.credentials.json),
// it unsets the long-lived CLAUDE_CODE_OAUTH_TOKEN env var that cli's
// container-create path injected — so the credentials file wins and
// Remote Control / full-scope features work as expected. When no
// credentials file exists, the env var is left intact for inference-only
// flows (its original purpose). See cli issue #7.
func claudeTokenShadowScript() string {
	return `# Auto-generated by cw local create. Do not edit by hand.
# Unsets CLAUDE_CODE_OAUTH_TOKEN when ~/.claude/.credentials.json exists,
# so freshly-minted full-scope creds from ` + "`claude auth login`" + ` win
# over the long-lived inference-only token forwarded by cli.
